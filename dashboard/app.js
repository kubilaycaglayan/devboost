    ...
    function renderQuotaNavIndicator(accounts = usageAccounts, snapshots = usageSnapshots) {
      ...
      const fill = document.getElementById("quota-nav-meter-fill");
      const label = document.getElementById("quota-nav-meter-label");
      if (!meter || !fill || !label) return;
      const candidate = quotaNavCandidate(accounts, snapshots);
      const percent = candidate && usagePercent(candidate.quota);
      if (percent == null) {
        meter.hidden = true;
        label.textContent = "";
        return;
    ...
    function renderUsage(data) {
      ...
      renderQuotaNavIndicator(usageAccounts, usageSnapshots);
      ...
      body.innerHTML = usageAccounts.map(a => {
        ...
        const navQuotaIndex = a.provider === "codex"
          ? (() => { const fiveHour = (s.quotas || []).findIndex(quota => /5\s*[- ]?hour/i.test(String(quota.name || "")) && usagePercent(quota) != null); return fiveHour >= 0 ? fiveHour : (s.quotas || []).findIndex(quota => usagePercent(quota) != null); })()
          : (s.quotas || []).findIndex(quota => usagePercent(quota) != null);
        const navSelected = quotaNavSelection && quotaNavSelection.accountId === String(a.id || "");
        const navButton = navQuotaIndex >= 0 ? `<button type="button" class="usage-action-icon usage-nav-icon${navSelected ? " selected" : ""}" onclick="selectQuotaForNavigation(${accountId}, ${navQuotaIndex})" title="Show this quota in navigation" aria-label="Show this quota in navigation">▔</button>` : "";
        return `<tr data-usage-id="${escapeHtml(String(a.id || ""))}"><td>${escapeHtml(a.provider)}</td><td><span class="usage-account-name">${escapeHtml(a.name)}</span>${navButton}${source}</td><td>${quotas}</td><td>${balances}</td><td class="mono usage-age" data-last-valid-query="${escapeHtml(s.last_valid_query_at || (s.ok && !s.stale ? s.updated_at : ""))}">${usageUpdated(s)}</td><td><div class="usage-actions"><button type="button" class="usage-action-icon" onclick="editUsageAccount(${accountId})" title="Edit account" aria-label="Edit account">⚙</button><button type="button" class="usage-action-icon usage-remove-icon" onclick="removeUsageAccount(${accountId})" title="Remove account" aria-label="Remove account">✕</button><button type="button" class="usage-drag-handle" draggable="true" ondragstart="onUsageRowDragStart(event, ${accountId})" ondragend="onUsageRowDragEnd()" title="Drag to reorder" aria-label="Drag to reorder">⠿</button></div></td></tr>`;
      }).join("");
    }
